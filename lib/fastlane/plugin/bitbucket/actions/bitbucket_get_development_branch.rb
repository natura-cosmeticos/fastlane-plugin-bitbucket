...
module Fastlane
  module Actions
    class BitbucketGetDevelopmentBranchAction < Action
      def self.run(params)
        ...
        branching_model = Helper::BitbucketHelper.get_branching_model(auth_header, base_url, params[:project_key], params[:repo_slug])

        development_info = branching_model["development"]
        development_branch = development_info["branch"]["name"]
        development_branch
      end

      def self.description
        "This action allows fastlane to fetch if a repository has a development branch."
      end
      ...
      def self.return_value
        "This action returns the development branch of a repository"
      end
